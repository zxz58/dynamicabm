...
| File | What it adds |
|---|---|
| `PhaseDiagramSIRS_alpha.py` (+ `phase_diagram_alpha.py`) | An SIRS variant of the alpha-mutation phase diagram. Infected nodes recover into the `-2` compartment as in SIR, then lose immunity with probability `--omega` per step and become susceptible again. Because recovered is transient, the outbreak criterion uses final `I/N >= 0.2`, matching the endemic-state logic used by SIS. |
| `CommunitySIRS.py` (+ `community_sirs.py`) | A focused endemic SIRS ABM on 2 or 3 clustered Watts-Strogatz communities. It uses continuous-time Gillespie events, virulence-dependent transmission/recovery/mortality/behavior/mobility trade-offs, waning immunity, mutation at transmission, dynamic inter-community edges, and CSV steady-state summaries after burn-in. Add `--samples-out` to also write plotting-ready sampled trajectories. |
| `PlotCommunitySIRS.py`, `CommunitySIRSNetworkSnapshots.py` | Figure tools for the endemic SIRS model. `PlotCommunitySIRS.py` creates a four-panel steady-state trajectory plot from `--samples-out`; `CommunitySIRSNetworkSnapshots.py` runs one realization and saves selected-time network PNG snapshots plus metadata. |
