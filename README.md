...
| `CommunitySIRS.py` (+ `community_sirs.py`) | A focused endemic SIRS ABM on 2 or 3 clustered Watts-Strogatz communities. It uses continuous-time Gillespie events, virulence-dependent transmission/recovery/mortality/behavior/mobility trade-offs, waning immunity, mutation at transmission, dynamic inter-community edges, and CSV steady-state summaries after burn-in. |
